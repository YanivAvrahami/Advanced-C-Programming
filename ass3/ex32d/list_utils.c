...
void merge4Helper(ListNode *lst1Head, ListNode *lst2Head, List *output)
{
	...
	{
		insertDataToEndList(output, *(lst2Head->dataPtr));
		merge4Helper(lst1Head, lst2Head->next, output);
	}
	else if (lst2Head == NULL)
	{
		insertDataToEndList(output, *(lst1Head->dataPtr));
		merge4Helper(lst1Head->next, lst2Head, output);
	}
	else
	{
		if (*(lst1Head->dataPtr) > *(lst2Head->dataPtr))
